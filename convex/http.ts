...
// This page captures the code and redirects to the reqlite:// deep link
...
http.route({
  ...
  handler: httpAction(async (_ctx, request) => {
    ...
    if (code) {
      const deepLink = `reqlite://auth/callback?code=${encodeURIComponent(code)}`;
      const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>ReqLite - Completing Sign In</title>
  <style>
...
<body>
  ...
  <script>window.location.href = "${deepLink}";</script>
    ...
    return new Response("ReqLite API", { status: 200 });
  }),
});
